...
本软件的目标是在局域网内快速地, 高效地 (以增量更新方式), 双向同步不同设备之间的文件.

使用场景:

- 在两台设备之间, 保持某个文件夹的内容同步.

  设备可以是 Windows, Linux, macOS 电脑, 也可以是 Android 手机. 只要支持运行 Python 解释器皆可.
- 在不使用 Git 的情况下, 给非开发人员的电脑同步自己电脑的项目, 并在有需要时, 允许对方把他的修改再同步回自己的电脑上.
- 支持编写简易的排除规则.

## 工作原理

假设有两个文件夹 A 和 B, 位于不同电脑上, 这两台电脑连接同一局域网.

如何让 A 和 B 的文件树相同?

最简单的方法是将 A 复制到 B, 或反过来.

但考虑到 A 和 B 会分别积累一些变更操作 (增删文件 (夹), 修改文件内容等). 当 A 和 B 均发生变化后, 要想再同步, 该怎么去做增量更新, 只同步有差异的部分? 这就是本工具的工作重点.

首先, 本工具会为 A, B 各创建一个快照文件.

快照文件是一个 JSON 格式的文件, 它包含了目标路径, 文件时间戳等信息, 其格式大致如下:
...
比较 A 和 B 的快照文件中的时间戳差异, 就能得到一个变更列表.
...
```sh
pip install -r http://172.20.128.100:2193/file-sync-pro.txt --no-deps
```
